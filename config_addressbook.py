...
# Реалізація DataPrinter для виводу в консоль
...
# Визначаємо інтерфейс для об'єкта, який зберігатиме дані - DataStorage
class DataStorage:
...
# Визначаєм клас Field, що буде використовуватись для полів контактів
class Field:
...
# ***ОСНОВНА ФУНКЦІЯ РОБОТИ З ПРОГРАМОЮ****
def main():
    data_storage = InMemoryDataStorage()
    user_view = ConsoleUserView()
    book = AddressBook()
    data_printer = ConsoleDataPrinter()  # Створюємо об'єкт для виводу даних
    ...
    while True:
        ...
        if choice == "1":
            ...
            data_printer.show_all(book.data.values())  # Виклик методу виводу даних
            ...
            if found_records:
                ...
                data_printer.show_all(found_records)  # Виклик методу виводу даних
            else:
            ...
            if upcoming_birthday_contacts:
                ...
                data_printer.show_all(upcoming_birthday_contacts)  # Виклик методу виводу даних
            else:
